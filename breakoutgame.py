...
def draw_bricks(bricks):
    ...
    inner_color = (184, 51, 255)  # Color for the inner part of the brick
    border_color = WHITE  # Color for the border

    for brick in bricks:
        pygame.draw.rect(window, border_color, brick)
        inner_brick = (brick.x + border_size, brick.y + border_size,
                       brick.width - 2 * border_size, brick.height - 2 * border_size)
...
def init_bricks():
    ...
    for row in range(BRICK_ROWS):
        for col in range(BRICK_COLS):
            brick = pygame.Rect(col * (BRICK_WIDTH + 5), 30 + row * (BRICK_HEIGHT + 5), BRICK_WIDTH, BRICK_HEIGHT)
            bricks.append(brick)
    return bricks
